...
int main(int argc, char ** argv)
{
    ...
    // 
    // sem_init(ponteiro_semaforo, 0 para threads / 1 para processos, valor_inicial)
    sem_init(&mutex, 0, 1);
    sem_init(&wrt, 0, 1);
...
void * leitor(void * id)
{
    ...
    //
    sem_wait(&mutex); // Entra na região crítica para alterar o readcount
    readcount++;
    if (readcount == 1) {
...
void * escritor(void * id)
{
    ...
    //
    sem_wait(&wrt); // Aguarda liberação exclusiva para escrever
    ...
    sem_post(&wrt); // Libera o acesso após a escrita
